do local socket = require("socket") local cronned = load_from_file('data/isup.lua') local function save_cron(msg, url, delete)
  ...
  return 'Saved!' end local function is_up_socket(ip, port)
  ...
  if not conn then
    ...
    c:close()
    return true
  end end local function is_up_http(url)
  ...
  return true end local function isup(url)
  ...
  return result end local function cron()
